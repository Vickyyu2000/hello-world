...
SETVAR animTcl::setlist[] = {{"garbage", TCL_LINK_INT, (char *)&garbage},
                             {"", 0, (char *)NULL}};
...
void traceProc(ClientData clientDatat, Tcl_Interp *interp, int level, char *command, Tcl_CmdProc *cmdProc,
...
int animTcl::Command(ClientData clientData, Tcl_Interp *interp, int argc, myCONST_SPEC char **argv)
{
  ...
  else if (strcmp(argv[0], "system") == 0)
  {
    ...
    BaseSystem *tempSystem = GlobalResourceManager::use()->getSystem(argv[1]);

    if (tempSystem != NULL)
    {
      tempSystem->command(argc - 2, &argv[2]);
    }
    else
    {
  ...
  else if (strcmp(argv[0], "simulator") == 0)
  {
    ...
    BaseSimulator *tempSimulator = GlobalResourceManager::use()->getSimulator(argv[1]);

    if (tempSimulator != NULL)
    {
      tempSimulator->command(argc - 2, &argv[2]);
    }
    else
    {
  ...
  else if (strcmp(argv[0], "show") == 0)
  {
    ...
    if (strcmp(argv[1], "simulator") == 0)
    {
      ...
      for (index = 0; index < GlobalResourceManager::use()->getNumberOfSimulators(); index++)
      {

        std::string name;

        GlobalResourceManager::use()->getSimulatorFromIndex(index)->getName(name);
    ...
    else if (strcmp(argv[1], "system") == 0)
    {
      ...
      for (index = 0; index < GlobalResourceManager::use()->getNumberOfSystems(); index++)
      {

        std::string name;

        GlobalResourceManager::use()->getSystemFromIndex(index)->getName(name);
