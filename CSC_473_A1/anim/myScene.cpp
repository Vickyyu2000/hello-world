...
static int part1GlobalCommand(ClientData clientData, Tcl_Interp* interp, int argc, myCONST_SPEC char** argv)
{
	...
	bool success;
	// retister hermite spline
	BaseSystem* hermite = new HermiteSpline("hermite");
	success = GlobalResourceManager::use()->addSystem(hermite, true);
	assert(success);
		 
	animTcl::OutputResult("Started part 1.");
	return TCL_OK;
}
...
static int part2GlobalCommand(ClientData clientData, Tcl_Interp* interp, int argc, myCONST_SPEC char** argv)
{
	...
	bool success;
	// retister hermite spline
	BaseSystem* objectpath = new HermiteSpline("objectpath");
	success = GlobalResourceManager::use()->addSystem(objectpath, true);
	assert(success);
	
	//
	MyModel* tank = new MyModel("tank");
	success = GlobalResourceManager::use()->addSystem(tank, true);
	assert(success);


	char* args[2] = { "read","data/tank.obj" };
	tank->command(2, args);

	// 
	PathSimulator* pathSimulator = new PathSimulator("path1", tank, objectpath);
	success = GlobalResourceManager::use()->addSimulator(pathSimulator);
	assert(success);

	animTcl::OutputResult("Started part 2.");
